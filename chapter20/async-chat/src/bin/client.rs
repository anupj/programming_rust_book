...
fn parse_command(line: &str) -> Option<FromClient> {
    ...
    if command == "post" {
        ...
        eprintln!("Unrecognised command: {:?}", line);
        return None;
    }
}
...
/// This function takes a socket receiving data from the server,
/// wraps a BufReader around it (note well, the async_std version),
/// and then passes that to receive_as_json to obtain a
/// stream of incoming FromServer values.
async fn handle_replies(from_server: net::TcpStream) -> ChatResult<()> {
    // Wrap the `TcpStream` in a `BufReader`
    let buffered = io::BufReader::new(from_server);
    // Get a `Stream<ChatResult>`
...
/// Given a string `input`, return `Some((token, rest))`, where
/// `token` is the first run of non-whitespace characters in
/// `input`, and `rest` is the rest of the string. If the string
/// contains no non-whitespace characters, return `None`.
